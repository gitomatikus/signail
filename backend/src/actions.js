...
// Transport-agnostic game actions. Both the WebSocket message handler and the
// SSE actions endpoint route here. The transport resolves the context:
//   ctx.isHost - host token already verified (WS query string / X-Host-Token)
//   ctx.conn   - the live connection object presence is keyed by (a ws socket
//                or an SSE stream client), or null when the sender has no
//                stream registered.
...
function handleAction(game, ctx, type, data) {
  if (type === 'user_login') {
    const userData = data;
    // Presence is keyed by the connection so it can be cleaned up when the
    // connection dies; without one the login could never be undone
    if (!userData || !ctx.conn) return;
    if (!userData.id) {
    ...
    game.onlineUsers.set(ctx.conn, userData);
    ...
    if (ctx.conn) {
      game.onlineUsers.delete(ctx.conn);
    }
    ...
    if (ctx.isHost && game.status === 'lobby') {
      game.status = 'started';
      game.broadcast({ type: 'game_started', data: { gameId: game.id } });
      game.broadcastGameInfo();
    }
  } else if (type === 'question_select') {
    ...
    game.broadcast({ type: 'question_select', data });
    game.broadcastSelectedQuestions();
  } else if (type === 'question_reveal') {
    game.revealedQuestions.add(data.questionId);
    game.broadcast({ type: 'question_reveal', data });
  } else if (type === 'answer_reveal') {
    game.broadcast({ type: 'answer_reveal', data });
  } else if (type === 'response_reveal') {
    game.broadcast({ type: 'response_reveal', data });
  } else if (type === 'return_to_game') {
    ...
    if (!questionTimes.has(userId)) {
      questionTimes.set(userId, data.elapsedTime);
      game.broadcast({ type: 'elapsed_time', data });
    }
  } else if (type === 'clear_selected_questions') {
    game.selectedQuestions.clear();
    game.broadcastSelectedQuestions();
  } else if (type === 'admin_clicked_red_number') {
    game.broadcast({ type: 'admin_clicked_red_number', data });
  } else if (type === 'admin_clicked_green_number') {
    game.lastGreenFrameUser = data.userId;
    game.broadcast({ type: 'admin_clicked_green_number', data });
  } else if (type === 'cat_clicks') {
    const { questionId, userId, clicksLeft } = data;
    ...
    const { questionId, userId, value } = data;
    if (userId && isValidAnswerValue(value) && !game.revealedAnswers.has(questionId)) {
  ...
  } else if (type === 'reveal_number_answers') {
  ...
  } else if (type === 'secret_assign') {
    const { questionId, targetUserId, selectorUserId } = data;
...
module.exports = { handleAction, isValidAnswerValue };
